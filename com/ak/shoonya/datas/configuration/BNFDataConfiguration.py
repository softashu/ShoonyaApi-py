...
def getBankNiftyExpiryDate():
    ...
    banknifty_expiry = {
        ...
        # weekly
        datetime.datetime(currentDateTime.year, 6, 19).date(): "19JUN24",
        datetime.datetime(currentDateTime.year, 7, 3).date(): "03JUL24",
        datetime.datetime(currentDateTime.year, 7, 10).date(): "10JUL24",
        datetime.datetime(currentDateTime.year, 7, 16).date(): "16JUL24",

        # monthly
        datetime.datetime(currentDateTime.year, 6, 26).date(): "26JUN24",
        datetime.datetime(currentDateTime.year, 7, 31).date(): "31JUL24",
        datetime.datetime(currentDateTime.year, 8, 28).date(): "28AUG24",
        datetime.datetime(currentDateTime.year, 9, 25).date(): "25SEP24",
        datetime.datetime(currentDateTime.year, 10, 30).date(): "30OCT24",
        datetime.datetime(currentDateTime.year, 11, 27).date(): "27NOV24",
        datetime.datetime(currentDateTime.year, 12, 24).date(): "24DEC24"
    }
...
# Method used to build and return the list of bank nifty option symbols that may use for data subscription
def buildAndGetBNFOptionSymbols(ltp):
    bnf_strikes = []
    bnf_option_symbols = []
    a = float(ltp['lp'])
    bnf_current_expiry = getBankNiftyExpiryDate()
    for i in range(-5, 5):
        strike = (int(a / 100) + i) * 100
        bnf_strikes.append(strike)

    # Add CE
    for strike in bnf_strikes:
        ltp_option = "NFO:BANKNIFTY" + str(bnf_current_expiry) + "C" + str(strike)
        bnf_option_symbols.append(ltp_option)

    # Add PE
    for strike in bnf_strikes:
        ltp_option = "NFO:BANKNIFTY" + str(bnf_current_expiry) + "P" + str(strike)
        bnf_option_symbols.append(ltp_option)

    return bnf_option_symbols
